...
const registerUser = asyncHandler(async (req, res,next) => {
  ...
  await user.save();
  const mailContent = emailVerificationMailgenContent(fullName,`http://localhost:${process.env.PORT}/verifyEmail/${token.unHashedToken}`)
  await sendEmail({email, subject: "Verify Email" ,mailgenContent: mailContent });
  if(user){
    req.apiResponse = {
...
const loginUser = asyncHandler(async (req, res,next) => {
  ...
  if(!response) throw new ApiError(status.UNAUTHORIZED,"User does not exist or invalid credentials");
  const isPaswordCorrect = response ? await response.isPasswordCorrect(password) : false;
  if(!isPaswordCorrect) throw new ApiError(status.UNAUTHORIZED,"Incorrect Password");
...
const logoutUser = asyncHandler(async (req, res,next) => {
  ...
  const response = await User.getUserByEmailOrUserName({email,username,_id});
  if(!!response){
...
const verifyEmail = asyncHandler(async (req, res,next) => {
  ...
  const response = await User.getUserByEmailOrUserName({emailVerificationToken:hashedToken , emailVerificationExpiry: {  $gt:  new Date().toUTCString() } , isEmailVerified:false});
  if(response){
...
const resendEmailVerification = asyncHandler(async (req, res,next) => {
  ...
  let response = null;
  if(!!email && !!username){
    response = await User.getUserByEmailOrUserName({email,username,isEmailVerified:false});
  }
  else if(!!email && !username){
    response = (email && await User.getUserByEmailOrUserName({email,isEmailVerified:false}))
  }
  else if(!!username && !email){
    response = (email && await User.getUserByEmailOrUserName({username,isEmailVerified:false}))
  }
  if(!response) throw new ApiError(status.UNAUTHORIZED,"User does not exist or invalid credentials");
  ...
  await response.save();
  const mailContent = emailVerificationMailgenContent(response.fullName,`http://localhost:${process.env.PORT}/verifyEmail/${token.unHashedToken}`)
  await sendEmail({email, subject: "Reverify Email" ,mailgenContent: mailContent });
  req.apiResponse = {
    statusCode: status.ACCEPTED,
    data:{ success: true },
...
const resetForgottenPassword = asyncHandler(async (req, res,next) => {
  ...
  if(!response) throw new ApiError(status.UNAUTHORIZED,"User does not exist.");

  const token = await User.generateToken("temporary");
  ...
  const mailContent = forgotPasswordMailgenContent(response.fullName,`http://localhost:${process.env.PORT}/verifyResetPassword/${token.unHashedToken}`);
  await sendEmail({email:response.email, subject: "Reset Password" ,mailgenContent: mailContent });
